...
from pii_utils import (
    detect_and_deidentify_record, get_available_presets, PII_HANDLERS, PATTERN_PRESETS
)
# Import the UI content
import ui_content as text

class Worker(QObject):
    # ... (Worker class remains unchanged) ...
    finished = Signal(object)
    error = Signal(str)
    def _init_(self, input_path, patterns, mask_configs, expected_counts):
        super()._init_()
    ...
    def run(self):
        try:
            ...
            if file_ext == '.csv': headers, input_rows = self._read_csv()
                ...
                if fitz is None: raise ImportError(text.MISSING_LIB_MSG)
...
class PiiApp(QMainWindow):
    ...
    def _init_ui(self):
        ...
        title = QLabel(text.HEADER_TITLE); title.setFont(QFont("Segoe UI", 16, QFont.Bold)); title.setStyleSheet("color: white;")
        subtitle = QLabel(text.HEADER_SUBTITLE); subtitle.setFont(QFont("Segoe UI", 10)); subtitle.setStyleSheet("color: white;")
        header_layout.addWidget(title); header_layout.addWidget(subtitle); main_layout.addWidget(header)
        controls_layout = QHBoxLayout(); select_button = QPushButton(text.SELECT_FILE_BTN); select_button.clicked.connect(self._select_file)
        self.path_label = QLabel(text.NO_FILE_SELECTED_LBL); self.path_label.setStyleSheet("font-style: italic; color: #555;")
        self.run_button = QPushButton(text.RUN_BTN); self.run_button.setObjectName("Accent"); self.run_button.clicked.connect(self._run_detection)
        ...
        regex_group = QGroupBox(text.REGEX_GROUP_TITLE); regex_layout = QVBoxLayout(regex_group)
        mode_layout = QHBoxLayout(); self.preset_radio = QRadioButton(text.PRESET_RADIO); self.custom_radio = QRadioButton(text.CUSTOM_RADIO)
        ...
        next_button = QPushButton(text.NEXT_BTN); next_button.setObjectName("Nav"); next_button.clicked.connect(lambda: self.config_stack.setCurrentIndex(1))
        ...
        mask_group = QGroupBox(text.DEID_GROUP_TITLE); mask_layout = QGridLayout(mask_group)
        mask_layout.addWidget(QLabel(text.PII_TYPE_HEADER), 0, 0); mask_layout.addWidget(QLabel(text.MASK_HEADER), 0, 1); mask_layout.addWidget(QLabel(text.STRATEGY_HEADER), 0, 2); mask_layout.addWidget(QLabel(text.CHAR_HEADER), 0, 3); mask_layout.addWidget(QLabel(text.EXPECTED_HEADER), 0, 4)
        for i, key in enumerate(self.pii_types):
            ...
            self.mask_config_widgets[key]["enabled"].setChecked(True); self.mask_config_widgets[key]["strategy"].addItems(text.MASKING_STRATEGIES)
        ...
        back_button = QPushButton(text.BACK_BTN); back_button.setObjectName("Nav"); back_button.clicked.connect(lambda: self.config_stack.setCurrentIndex(0))
        ...
        self.preview_table = QTableWidget(); self.tabs.addTab(self.summary_text, text.SUMMARY_TAB); self.tabs.addTab(self.preview_table, text.PREVIEW_TAB)
        right_layout.addWidget(self.tabs)
        bottom_layout = QHBoxLayout(); save_csv_button = QPushButton(text.SAVE_DEIDENTIFIED_BTN); save_summary_button = QPushButton(text.SAVE_SUMMARY_BTN)
    ...
    def _select_file(self):
        path, _ = QFileDialog.getOpenFileName(self, text.SELECT_FILE_DIALOG_TITLE, "", text.FILE_FILTER)
        if path:
            ...
            if self.input_file_type == '.pdf' and fitz is None:
                QMessageBox.critical(self, text.MISSING_LIB_TITLE, text.MISSING_LIB_MSG); self.input_path = None

    def _run_detection(self):
        if not self.input_path: QMessageBox.warning(self, text.NO_FILE_WARNING_TITLE, text.NO_FILE_WARNING_MSG); return
    ...
    def _on_detection_complete(self, result):
        ...
        self._render_summary(); self._render_preview(); self.progress_bar.setVisible(False); self.run_button.setEnabled(True); QMessageBox.information(self, text.COMPLETED_INFO_TITLE, text.COMPLETED_INFO_MSG)
    ...
    def _compile_overrides(self) -> Dict[str, re.Pattern]:
        ...
        for key, widget in self.regex_widgets.items():
            ...
            if pattern_text:
                ...
                except re.error as e: QMessageBox.warning(self, text.REGEX_ERROR_TITLE, text.REGEX_ERROR_MSG.format(pii_label=self.pii_labels[key], error=e))
        return compiled

    def _render_summary(self):
        report_str = text.get_summary_report(self.summary, self.report_metrics, self.pii_labels)
        self.summary_text.setText(report_str)
    ...
    def _save_deidentified(self):
        if not self.deidentified_rows: QMessageBox.warning(self, text.NO_DATA_SAVE_TITLE, text.NO_DEID_DATA_MSG); return
        base_name = os.path.splitext(os.path.basename(self.input_path))[0] if self.input_path else "output"
        if self.input_file_type == '.csv':
            path, _ = QFileDialog.getSaveFileName(self, text.SAVE_CSV_DIALOG_TITLE, f"{base_name}_deidentified.csv", "CSV Files (*.csv)")
            if not path: return
            try:
                ...
                QMessageBox.information(self, "Success", text.SAVE_SUCCESS_MSG.format(path=path))
            except Exception as e: QMessageBox.critical(self, "Error", text.SAVE_FAILURE_MSG.format(error=e))
        else:
            path, _ = QFileDialog.getSaveFileName(self, text.SAVE_TXT_DIALOG_TITLE, f"{base_name}_deidentified.txt", "Text Files (*.txt)")
            if not path: return
            try:
    ...
    def _save_summary(self):
        if not self.summary: QMessageBox.warning(self, text.NO_DATA_SAVE_TITLE, text.NO_SUMMARY_DATA_MSG); return
        path, _ = QFileDialog.getSaveFileName(self, text.SAVE_SUMMARY_DIALOG_TITLE, "summary_report.txt", "Text Files (*.txt)")
        if not path: return
        try:
    ...
    def closeEvent(self, event: QCloseEvent):
        if self.thread and self.thread.isRunning():
            reply = QMessageBox.question(self, text.CONFIRM_EXIT_TITLE, text.CONFIRM_EXIT_MSG,
